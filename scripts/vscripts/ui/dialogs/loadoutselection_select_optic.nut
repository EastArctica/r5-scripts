...
void function ClientToUI_LoadoutSelectionOptics_OpenSelectOpticDialog( int loadoutIndex )
{
	...
	float xBounds = ( ruiScreenWidth / 2 ) - ( widgetWidth/2 )
	float yBounds = ( ruiScreenHeight / 2 ) - ( widgetHeight/2 )
	vector cursorPos = GetCursorPosition()

	                           
	var opticAnchor = Hud_GetChild( file.menu, "OpticAnchor" )
	Hud_SetX( opticAnchor, clamp( ( cursorPos.x - ruiScreenWidth * 0.5 ), -1 * xBounds, xBounds )  * xScale )
	Hud_SetY( opticAnchor, clamp( ( cursorPos.y - ruiScreenHeight * 0.5 ), -1 * yBounds, yBounds ) * yScale )
