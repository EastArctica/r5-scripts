...
void function UpdateFriendButtons()
{
	...
	foreach ( PartyMember partyMember in party.members )
	{
		if ( partyMember.uid == GetPlayerUID() )
		{
			...
			if(blockTimeLeft > 0)
			{
				toolTipData.descText = format( Localize( "#TOXIC_CHAT_BLOCKED" ), blockTimeLeft ).slice(7)
			}
...
bool function Lobby_ShowBattlePassPopup( bool forceShow = false )
{
	...
	{
		array<string> popupLevelMarkers_stringArray = split( popupLevelMarkers_stringOverride, " " )
