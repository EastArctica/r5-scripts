...
void function InitControlSpawnMenu( var newMenuArg )                                               
{
	...
	AddMenuFooterOption( menu, LEFT, BUTTON_DPAD_DOWN, false, "#CONTROL_SHOW_SCOREBOARD_GAMEPAD", "#CONTROL_SHOW_SCOREBOARD", ControlSpawnMenu_OpenScoreboard, CanOpenScoreboard )
	AddMenuFooterOption( menu, LEFT, BUTTON_DPAD_DOWN, false, "#CONTROL_HIDE_SCOREBOARD_GAMEPAD", "#CONTROL_HIDE_SCOREBOARD", ControlSpawnMenu_CloseScoreboard, CanCloseScoreboard )
	AddMenuFooterOption( menu, LEFT, BUTTON_A, false, "#CONTROL_REPORT_PLAYERS_GAMEPAD", "#CONTROL_REPORT_PLAYERS", ControlSpawnMenu_OpenReportPlayer, CanCloseScoreboard )

	AddMenuFooterOption( menu, RIGHT, BUTTON_DPAD_UP, false, "#MODE_DETAILS_GAMEPAD", "#MODE_DETAILS", ControlSpawnMenu_OpenGameModeDetails )
...
void function RegisterCallbacks()
{
	if( !file.inputRegistered )
	{
		...
		RegisterButtonPressedCallback( KEY_Z, ControlSpawnMenu_ToggleScoreboard )
		RegisterButtonPressedCallback( KEY_F2, ControlSpawnMenu_OpenGameModeDetails )
		RegisterButtonPressedCallback( KEY_SPACE, ControlSpawnMenu_OpenReportPlayer )
...
void function DeregisterCallbacks()
{
	if( file.inputRegistered )
	{
		...
		DeregisterButtonPressedCallback( KEY_Z, ControlSpawnMenu_ToggleScoreboard )
		DeregisterButtonPressedCallback( KEY_F2, ControlSpawnMenu_OpenGameModeDetails )
		DeregisterButtonPressedCallback( KEY_SPACE, ControlSpawnMenu_OpenReportPlayer )
...
void function ControlSpawnMenu_OpenCharacterSelect( var button )
{
	...
	else if ( UITime() > file.menuOpenTime + 0.1 )
	{
		...
		if( file.scoreboardOpened )
			RunClientScript( "Teams_HideScoreboard" )
...
{
	printt("ControlSpawnMenu_ToggleScoreboard", file.scoreboardOpened, file.hasMenuBeenOpened)
...
{
	printt("ControlSpawnMenu_OpenScoreboard")
	file.scoreboardOpened = true
	RunClientScript( "Teams_ShowScoreboard", FULLMAP_Z_BASE, 0 , 0.2, false, 0.0 )
...
void function ControlSpawnMenu_OpenReportPlayer( var button )
{
	if ( !IsFullyConnected() )
		return
	if ( !file.scoreboardOpened  )
	...
	if ( CanRunClientScript() )
		 RunClientScript( "UICallback_Control_ReportMenu_OnOpened" )

	 OpenTeamsScoreboardReportMenu()
}
		...
		if(!file.scoreboardOpened)
		{
			...
			foreach( button, waypointData in file.buttonToWaypointData )
			{
				...
				if ( rui != null )
				{
							...
							if ( spawnHeaderRui != null )
							{
								if ( waypointData.isFOB )
								{
									...
									if ( isFOBForLocalPlayer )
									{
										...
										RuiSetBool( spawnHeaderRui, "canSpawnOnFOB", waypointData.isUsable == 1 )
									}
								...
								else
								{
									...
									RuiSetBool( spawnHeaderRui, "canSpawnOnCentral", waypointData.isUsable == 1 )
					...
					bool isUsable = waypointData.isUsable >= 1 ? true : false
...
void function SetSpawnButtonTooltipData( var button )
{
	...
	else if ( buttonData.waypointType != CONTROL_WAYPOINT_BASE0_INDEX && buttonData.waypointType != CONTROL_WAYPOINT_BASE1_INDEX )                                                                                     
	{
		...
		else if ( buttonData.currentOwner == -1 )
		{
