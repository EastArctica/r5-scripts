...
void function SetupNarrativeDebugDevMenu()
{
	...
	SetupDevMenu( "Banglore Story Event Tease", SetDevMenu_BSETeaseDebug )
      
              
	SetupDevMenu( "Tropics MU1 Tease", SetDevMenu_TropicsMU1TeaseDebug )
...
void function SetDevMenu_BSETeaseDebug( var _ )
{
	ChangeToThisMenu( SetupBSETeaseDebug )
}

void function SetupBSETeaseDebug()
{
	SetupDevCommand("Launch Firing Range with Story", "launchplaylist survival_s12e04_frchallenges" )
	SetupDevCommand( "Go To Finale Area", "launchplaylist survival_s12e04_finale" )
	SetupDevCommand( "Fake Complete Challenge 1", "script DEV_FRCCompleteChallenge(GP(), \"challenge_s12e04_group1challenge0\", 0)" )
	SetupDevCommand( "Fake Complete Challenge 2", "script DEV_FRCCompleteChallenge(GP(), \"challenge_s12e04_group2challenge0\", 1)" )
	SetupDevCommand( "Fake Complete Challenge 3", "script DEV_FRCCompleteChallenge(GP(), \"challenge_s12e04_group3challenge0\", 2)" )
	SetupDevCommand( "Play Radio Vignette 1", "script_ui thread PlayVideoMenu( false, \"radio_vignette/s12e04/s12e04_challenge1\", \"diag_mp_se_baStoryEvent1\", eVideoSkipRule.INSTANT )" )
	SetupDevCommand( "Play Radio Vignette 2", "script_ui thread PlayVideoMenu( false, \"radio_vignette/s12e04/s12e04_challenge2\", \"diag_mp_se_baStoryEvent2\", eVideoSkipRule.INSTANT )" )
	SetupDevCommand( "Play Radio Vignette 3", "script_ui thread PlayVideoMenu( false, \"radio_vignette/s12e04/s12e04_challenge3\", \"diag_mp_se_baStoryEvent3\", eVideoSkipRule.INSTANT )" )
}
      

              
void function SetDevMenu_TropicsMU1TeaseDebug( var _ )
{
	ChangeToThisMenu( TropicsMU1TeaseDebug )
}

void function TropicsMU1TeaseDebug()
{
	float currentFakeTimeDays = GetConVarFloat( "test_fakeTimeDays")
	int now = GetUnixTimestamp()

	int phase1ActivationTime = expect int( GetCurrentPlaylistVarTimestamp( "s12e05_phase1", UNIX_TIME_FALLBACK_2038 ) )
	float phase1FakeTimeDays = (float(phase1ActivationTime) - float(now)) / float( SECONDS_PER_DAY ) + currentFakeTimeDays + 0.5
	SetupDevCommand( "Phase 1", "test_fakeTimeDays " + phase1FakeTimeDays + "; map mp_rr_tropic_island" )

	int phase2ActivationTime = expect int( GetCurrentPlaylistVarTimestamp( "s12e05_phase2", UNIX_TIME_FALLBACK_2038 ) )
	float phase2FakeTimeDays = (float(phase2ActivationTime) - float(now)) / float( SECONDS_PER_DAY ) + currentFakeTimeDays + 0.5
	SetupDevCommand( "Phase 2", "test_fakeTimeDays " + phase2FakeTimeDays + "; map mp_rr_tropic_island" )

	int phase3ActivationTime = expect int( GetCurrentPlaylistVarTimestamp( "s12e05_phase3", UNIX_TIME_FALLBACK_2038 ) )
	float phase3FakeTimeDays = (float(phase3ActivationTime) - float(now)) / float( SECONDS_PER_DAY ) + currentFakeTimeDays + 0.5
	SetupDevCommand( "Phase 3", "test_fakeTimeDays " + phase3FakeTimeDays + "; map mp_rr_tropic_island" )

	int phase4ActivationTime = expect int( GetCurrentPlaylistVarTimestamp( "s12e05_phase4", UNIX_TIME_FALLBACK_2038 ) )
	float phase4FakeTimeDays = (float(phase4ActivationTime) - float(now)) / float( SECONDS_PER_DAY ) + currentFakeTimeDays + 0.5
	SetupDevCommand( "Phase 4", "test_fakeTimeDays " + phase4FakeTimeDays + "; map mp_rr_tropic_island" )

	int phase5ActivationTime = expect int( GetCurrentPlaylistVarTimestamp( "s12e05_phase5", UNIX_TIME_FALLBACK_2038 ) )
	float phase5FakeTimeDays = (float(phase5ActivationTime) - float(now)) / float( SECONDS_PER_DAY ) + currentFakeTimeDays + 0.5
	SetupDevCommand( "Phase 5", "test_fakeTimeDays " + phase5FakeTimeDays + "; map mp_rr_tropic_island" )
}
