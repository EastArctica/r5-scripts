...
ButtonVars function GetBindDisplayName( string bind )
{
	...
	switch ( bind.tolower() )
	{
		...
		case "scriptcommand1":
			...
			break
		case "+scriptCommand7":
			displayName.common = "#CHARACTER_INFO"
			displayName.pilot = ""
			displayName.titan = ""
			break
...
void function RefreshButtonBinds( int presetIndex = -1 )
{
	...
	else if( IsGamepadPS5() )
	{
		RuiSetImage( file.gamepadLayoutRui, "gamepadImage", $"rui/menu/controls_menu/ps5_gamepad_button_layout" )
	}
	else if ( IsGamepadXB5() )
	{
		RuiSetImage( file.gamepadLayoutRui, "gamepadImage", $"rui/menu/controls_menu/xbx_gamepad_button_layout" )
	}
	else if ( IsGamepadNX() )
