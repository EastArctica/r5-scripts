...
const string LOADOUTSELECTIONMENU_FEATUREDLOADOUT_BUTTON = "FeaturedLoadoutSelectButton"
const string LOADOUTSELECTIONMENU_FEATUREDLOADOUTWEAPON_BUTTON = "FeaturedLoadout"
const string LOADOUTSELECTIONMENU_CHALLENGELOADOUT_BUTTON = "ChallengeLoadoutSelectButton"
const string LOADOUTSELECTIONMENU_CHALLENGELOADOUTWEAPON_BUTTON = "ChallengeLoadout"
...
struct
{
	...
	int maxLoadoutCount = -1
                     
		int maxLoadoutCountFeatured = -1
		int maxLoadoutCountChallenge = -1
...
void function LoadoutSelectionMenu_InitLoadoutMenu( var newMenuArg )
{
	...
	if ( file.maxLoadoutCount == -1 )
		file.maxLoadoutCount = LOADOUTSELECTION_MAX_LOADOUT_COUNT

                     
		if ( file.maxLoadoutCountFeatured == -1 )
			file.maxLoadoutCountFeatured = LOADOUTSELECTION_MAX_LOADOUT_COUNT_FEATURED

		if ( file.maxLoadoutCountChallenge == -1 )
			file.maxLoadoutCountChallenge = LOADOUTSELECTION_MAX_LOADOUT_COUNT_CHALLENGE
...
{
	LoadoutSelectionMenu_RefreshLoadouts( file.maxLoadoutCount, eLoadoutSelectionSlotType.REGULAR )
                     
		                                                      
		LoadoutSelectionMenu_RefreshLoadouts( file.maxLoadoutCountFeatured, eLoadoutSelectionSlotType.FEATURED )

		                                                       
		LoadoutSelectionMenu_RefreshLoadouts( file.maxLoadoutCountChallenge, eLoadoutSelectionSlotType.CHALLENGE )
...
void function LoadoutSelectionMenu_RefreshLoadouts( int maxLoadouts, int loadoutSlotType )
{
	...
	{
		float diff = float( LOADOUTSELECTION_MAX_LOADOUT_COUNT - loadoutCount )
...
void function UpdateFocusedLoadoutButtonGroups()
{
	UpdateFocusedLoadout( eLoadoutSelectionSlotType.REGULAR )
		...
		UpdateFocusedLoadout( eLoadoutSelectionSlotType.FEATURED )

		                                                       
		UpdateFocusedLoadout( eLoadoutSelectionSlotType.CHALLENGE )
...
{
	bool shouldUpdateMaxVal = maxLoadouts != file.maxLoadoutCount ? true : false
                     
		if ( LoadoutSelection_AreFeaturedLoadoutsEnabled() && loadoutType ==  eLoadoutSelectionSlotType.FEATURED )
			shouldUpdateMaxVal = maxLoadouts != file.maxLoadoutCountFeatured ? true : false

		if ( LoadoutSelection_AreChallengeLoadoutsEnabled() && loadoutType == eLoadoutSelectionSlotType.CHALLENGE )
			shouldUpdateMaxVal = maxLoadouts != file.maxLoadoutCountChallenge ? true : false
	...
	if ( shouldUpdateMaxVal )
	{
		...
		int loadoutButtonCount = LoadoutSelectionMenu_GetLoadoutButtonCountByLoadoutSlotType( loadoutType )
			...
			{
				if ( LoadoutSelection_AreFeaturedLoadoutsEnabled() && loadoutType ==  eLoadoutSelectionSlotType.FEATURED )
					file.maxLoadoutCountFeatured = maxLoadouts

				if ( LoadoutSelection_AreChallengeLoadoutsEnabled() && loadoutType == eLoadoutSelectionSlotType.CHALLENGE )
					file.maxLoadoutCountChallenge = maxLoadouts
			}
			else
                            
			{
				file.maxLoadoutCount = maxLoadouts
			}
...
void function LoadoutSelectionMenu_ResetLoadoutButtons()
{
	...
	int loadoutSlotIndex = 0
		...
		int featuredLoadoutSlotIndex = 0
		int challengeLoadoutSlotIndex = 0
	...
	for ( int i = 0; i < LOADOUTSELECTION_MAX_TOTAL_LOADOUT_SLOTS; i++ )
	{
		...
		else if ( LoadoutSelection_AreFeaturedLoadoutsEnabled() && loadoutSlotType == eLoadoutSelectionSlotType.FEATURED )
		{
			LoadoutSelectionMenu_SetupLoadoutButton( featuredLoadoutSlotIndex, i, loadoutSlotType )
			featuredLoadoutSlotIndex++
		}
		else if ( LoadoutSelection_AreChallengeLoadoutsEnabled() && loadoutSlotType == eLoadoutSelectionSlotType.CHALLENGE )
		{
			LoadoutSelectionMenu_SetupLoadoutButton( challengeLoadoutSlotIndex, i, loadoutSlotType )
			challengeLoadoutSlotIndex++
		}
...
string function LoadoutSelectionMenu_GetLoadoutButtonPrefix( int loadoutSlotType )
{
	...
	switch ( loadoutSlotType )
	{
		...
		case eLoadoutSelectionSlotType.FEATURED:
			prefix = LOADOUTSELECTIONMENU_FEATUREDLOADOUT_BUTTON
			break
		case eLoadoutSelectionSlotType.CHALLENGE:
			prefix = LOADOUTSELECTIONMENU_CHALLENGELOADOUT_BUTTON
			break
...
string function LoadoutSelectionMenu_GetWeaponButtonPrefix( int loadoutSlotType )
{
	...
	switch ( loadoutSlotType )
	{
		...
		case eLoadoutSelectionSlotType.FEATURED:
			prefix = LOADOUTSELECTIONMENU_FEATUREDLOADOUTWEAPON_BUTTON
			break
		case eLoadoutSelectionSlotType.CHALLENGE:
			prefix = LOADOUTSELECTIONMENU_CHALLENGELOADOUTWEAPON_BUTTON
			break
...
{
	int loadoutCount = file.maxLoadoutCount

                     
		switch ( loadoutSlotType )
		{
			case eLoadoutSelectionSlotType.FEATURED:
				loadoutCount = file.maxLoadoutCountFeatured
				break
			case eLoadoutSelectionSlotType.CHALLENGE:
				loadoutCount = file.maxLoadoutCountChallenge
				break
			default:
				break
		}
