...
void function AnimateXPBar( bool isFirstTime )
{
	...
	string killsAssistsString = Localize( "#RANKED_KILL_SCORE", numKills, numAssists, pointsPerKillForPlacement )
	...
	for (int i = 0 ; i < killsAssistsTier.len(); i++){
		...
		{
			int killPoints = GetKillPointsWithTierDiff(GetTierDifference(i, previousRank.tier.index)) * killsAssistsTier[i]
			additionalLineCount++
			killsAssistsString += "\n"
			killsAssistsString += Localize( "#RANKED_KILL_SCORE_" + i , killsAssistsTier[i], killPoints)
		}
	}

	int lineCount =  1 +  int (ceil(additionalLineCount * 0.5))
	RuiSetString( rui, "line2KeyString", killsAssistsString)
	RuiSetInt( rui, "line2RowHeight", lineCount )
	...
	if ( tierDerankingProtectionAdjustment > 0  )                                           
	{
		...
		else
		{
			if ( previousGameWasAbandonded )
			{
				RuiSetString( rui, "line5KeyString", "#RANKED_TIER_DERANKING" )
				RuiSetString( rui, "line5ValueString", string( score - previousScore + tierDerankingProtectionAdjustment) )
				RuiSetColorAlpha( rui, "line5Color", <1, 1, 1>, 1 )
				numLines = 5
			}
			else
			{
				...
				RuiSetString( rui, "line4KeyString", "#RANKED_TIER_DERANKING" )
				RuiSetString( rui, "line4ValueString", string( score - previousScore + tierDerankingProtectionAdjustment) )
				RuiSetColorAlpha( rui, "line4Color", <1, 1, 1>, 1 )
				numLines = 4
			}
		}
