	...
	if ( character != null && isFirstTime )
	{
		...
		foreach ( badge in allBadges )
		{
			...
			foreach ( tierData in tierDataList )
			{
				...
				{
					case eStatType.INT:
						int currentVal = GetStat_Int( GetLocalClientPlayer(), se, eStatGetWhen.CURRENT )
						if ( currentVal < tierData.unlocksAt )
							continue

						int previousVal = GetStat_Int( GetLocalClientPlayer(), se, eStatGetWhen.START_OF_PREVIOUS_MATCH )
					...
					case eStatType.FLOAT:
						float currentVal = GetStat_Float( GetLocalClientPlayer(), se, eStatGetWhen.CURRENT )
						if ( currentVal < tierData.unlocksAt )
							continue

						float previousVal = GetStat_Float( GetLocalClientPlayer(), se, eStatGetWhen.START_OF_PREVIOUS_MATCH )
						if ( previousVal >= tierData.unlocksAt )
							continue
