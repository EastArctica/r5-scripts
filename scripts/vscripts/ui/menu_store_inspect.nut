...
void function StoreInspectMenu_UpdatePurchaseButton( GRXScriptOffer storeOffer, StoreInspectOfferData offerData, StoreInspectUIData uiData )
{
	...
	if ( storeOffer.prereq != null )
	{
		...
		if ( ItemFlavor_GetType( prereqFlav ) == eItemType.calevent_collection )
		{
			offerData.isPurchasable = false
			if( !isOfferFullyClaimed && CollectionEvent_IsRewardMythicSkin( prereqFlav ) )
			...
			offerData.purchaseDescText = Localize( ( isOfferFullyClaimed ? "#STORE_REQUIRES_OWNED" : "#STORE_REQUIRES_LOCKED" ), Localize( CollectionEvent_GetCollectionName( prereqFlav ) ) )
