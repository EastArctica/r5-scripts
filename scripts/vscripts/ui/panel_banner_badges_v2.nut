	...
	else
	{
		if ( tierDataList.len() == 1 && tierDataList[0].unlocksAt > 0 )
		{
				...
				if ( se.type == eStatType.INT || se.type == eStatType.FLOAT )
				{
					...
					if ( se.type == eStatType.INT )
					{
						if ( goalVal == 1 )
							...
							currVal = float(GetStat_Int( GetLocalClientPlayer(), se ))
					}
					else if ( se.type == eStatType.FLOAT )
					{
						currVal = GetStat_Float( GetLocalClientPlayer(), se )
					}
					if ( good )
			...
			else if ( ItemFlavor_GetGRXMode( badge ) == eItemFlavorGRXMode.REGULAR )
			{
				...
				{
					ItemFlavor parentChallengeFlav = GetItemFlavorByAsset( GetGlobalSettingsAsset( ItemFlavor_GetAsset( badge ), "parentItemFlavor" ) )
