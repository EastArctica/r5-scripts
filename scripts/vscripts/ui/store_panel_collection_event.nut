...
{
	bool awardMythic = CollectionEvent_IsRewardMythicSkin( activeCollectionEvent )
	...
	ItemFlavor charFlav = expect ItemFlavor( GetItemFlavorAssociatedCharacterOrWeapon( CollectionEvent_GetPrimaryCompletionRewardItem( activeCollectionEvent ) ) )
...
void function UpdateFocusStuff( var focusedRewardButtonOrNull )
{
	...
	if ( focusedRewardButtonOrNull == null && CollectionEvent_IsRewardMythicSkin( activeCollectionEvent ) && file.WORKAROUND_currentlyFocusedRewardButtonForFooters == file.completionRewardBox )
	{
	...
	RuiSetBool( rui, "hasBonus", hasBonus )
	RuiSetString( rui, "bonusString", bonusString )
...
void function CompletionRewardBox_OnClick( var btn )
{
	...
	if ( CollectionEvent_IsRewardMythicSkin( activeCollectionEvent ) )
...
{
	if ( file.activeCollectionEvent != null )
		HudElem_SetRuiArg( file.completionRewardBox, "itemImage", CollectionEvent_GetMythicButtonImage( expect ItemFlavor( file.activeCollectionEvent ), file.activeMythicTier ) )
	HudElem_SetRuiArg( file.completionRewardBox, "activeMythicTier", file.activeMythicTier + 1 )
