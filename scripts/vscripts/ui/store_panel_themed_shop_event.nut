...
void function OfferButton_SetPrice( var button, GRXScriptOffer offer )
{
	...
	if ( offer.output.flavors.len() == 1 && ItemFlavor_GetType( offer.output.flavors[0] ) == eItemType.account_pack )
	{
		                                                                                                         
		packCollectionInfo = GRXOffer_GetEventThematicPackCollectionInfo( offer.output.flavors[0] )
